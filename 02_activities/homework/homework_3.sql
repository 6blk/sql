...
FROM (
	SELECT customer_last_name, customer_first_name, SUM(quantity * cost_to_customer_per_qty) as spent_amount
	FROM customer_purchases cp
	LEFT JOIN customer c
	ON cp.customer_id = c.customer_id
	GROUP BY cp.customer_id
	HAVING spent_amount > 2000
)	
...
SELECT
	...
	STRFTIME('%m', market_date) as MONTH,
	STRFTIME('%Y', market_date) as YEAR,
	SUM(quantity * cost_to_customer_per_qty) as spent_amount
FROM
	customer_purchases
WHERE
