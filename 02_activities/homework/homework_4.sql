...
SELECT
	customer_id
	,market_date
	,ROW_NUMBER() OVER (PARTITION BY customer_id ORDER BY market_date) as [Number customers visits]
...
SELECT
	customer_id
	,product_id
	,COUNT(product_id) as [purchased N times]
FROM customer_purchases
...
SELECT
	...
	CASE 
		...
		THEN SUBSTR(product_name, INSTR(product_name, ' - ')+3)
		ELSE ''
	END  AS description
FROM product
